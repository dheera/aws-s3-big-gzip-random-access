...
def build_index(s3_client, bucket_name, object_key):
...
def build_index_for_prefix(s3_client, bucket_name, in_prefix):
    ...
    for indexable in indexables:
        ...
        build_index(s3_client, bucket_name, in_prefix)

def get_stream(s3_client, bucket_name, object_key, indenx_file = None):
    assert(object_key.endswith(".gz"))
    ...
    if index_file is None: # assume it is where we would have created it with the build_index() function
        index_path = os.path.join(".gzidx", host, bucket_name, object_key.replace(".gz", ".gzidx"))
    
    assert(os.path.exists(index_path))

    return indexed_gzip.IndexedGzipFile(fileobj=seekable_stream, index_file=index_path)
